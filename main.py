...
def convert_raw_data_to_metadata():#原始数据转换为元数据
    print("Converting raw data to metadata...")
    with open(raw_data_file, 'rb') as f:
        reader = csv.reader(f)
        next(reader, None)  # skipping the header
        reader = sorted(reader, key=lambda row: (row[2], row[5]))  # Sort by cascade id sorted函数的应用
        row_num = 0#对于数据行的计数
        last_cascade_id = None
        # resetting files...
        fout = open(metadata_file, 'w')#初始化文件
        fout.close()
        for row in reader:
            tid = int(row[0])
            veracity = str(row[1])
            ...
            user_account_age = eval(row[7])#计算字符串，去掉引号
            ...
            ##级联的id,如果是之前的id就继续构造图，如果不是就提取数据，构成新的图
            if cascade_id != last_cascade_id:  # New cascade
                ...
                if last_cascade_id != None:
                    ...
                    fout.close()
                ###Creating a new graph using networkx to extract metadata，创建一个新的图
                metadata = {}
                ...
                last_tid = tid
            graph.add_node(tid, date=tweet_date, is_root=True if parent_tid == -1 else False, followers=user_followers,
            ...
            if row_num % 10000 == 0:
                print(row_num, "rows read")#没一万行报告一次
            row_num += 1#这样就一行不断的加

    # Saving the last remaining cascade
    mt = analyze_cascade.get_metadata(graph)
    metadata.update(mt)#mt是一个字典的格式，很方便分析使用，直接更新
    fout = open(metadata_file, 'a')
    fout.write(repr((last_cascade_id, metadata)) + '\n')
    fout.close()


def generate_figures():#由级联的数据生成图
    print("Reading metadata...")
    fin = open(metadata_file, 'r')
    lines = fin.readlines()
    fin.close()
    cascade_id2metadata = {}
    for line in lines:#这个是为啥
        line = line.replace('\n', '')#字符串格式的
        item = eval(line)#第0列对应第一列
    ...
    lines = fin.readlines()
    fin.close()
    regression_data: list[Any] = []  # [cluster,was_retweeted,folllowers,followees,verified,account_age,engagement,falsehood]
    for line in lines:
        line = line.replace('\n', '')
    ...
    with open(emotions_file, 'rb') as f:
        ...
        for row in reader:
            if row_num == 0:
                ...
                for col in range(1, len(header)):#header没有增加或改变
                    emotion = header[col]
    ...
    unique_rumors = set([])#谣言集合
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        veracity2number_of_cascades[veracity] = number_of_cascades#怪怪的
    print("Number of unique rumors:", len(unique_rumors))
    total_number_of_cascades = 0
    for veracity, number_of_cascades in list(veracity2number_of_cascades.items()):#到底在迭代啥，键值对
    ...
    ax.xaxis.set_major_formatter(formatter)#操作刻度的显示吗
    ax.yaxis.set_major_formatter(formatter)
    veracity2rid2count = {}
    for cascade, metadata in list(cascade_id2metadata.items()):#item返回一个二元组组成的键值对，再转化为列表，仅仅只是以这两个
        veracity = metadata['veracity']#作为键值对的命名
        rid = metadata['rumor_id']
        rid2count = veracity2rid2count.get(veracity, {})#如果有就返回，没有就返回空字典，注意到ver在不停的变化，所以是未了考察
        count = rid2count.get(rid, 0)     #以前是否出现过，出现过就获取rid对应的count，没有就是0,再把rid的count给记录上去
        count += 1
        rid2count[rid] = count
        veracity2rid2count[veracity] = rid2count#这个也是在不断的迭代中，对于不同的matadata有不同的键值对
    for veracity, rid2count in list(veracity2rid2count.items()):#在这个地方循环着呢，所以能画3条曲线
        rid_count = list(rid2count.items())
        rid_count.sort(key=lambda x: x[1], reverse=True)#元组的访问方式，按照值来排序对应于x1
        y = [count for rid, count in rid_count]
        total = float(len(rid_count))
        xf = []
        pf = []
        y.sort()#默认升序排序
        counts = list(set(y))#有多少种不同的y
        counts.sort()
        for d in counts:
            ind = y.index(d)#找到对应的位置
            count = len(y[ind:])#从这个角标往后的长度
            p = (count / float(total)) * 100#只是对应count在总体之中的位置，这个是正确的
            xf.append(d)
            pf.append(p)
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        veracity = metadata['veracity']
        date = quarter_year(metadata['start_date'])#自定义的函数吗还是,根据开始时间绘制，这一步对时间进行了转化
        date2count = veracity2date2count.get(veracity, {})
        count = date2count.get(date, 0)
        count += 1
        date2count[date] = count
        veracity2date2count[veracity] = date2count#值为data2count date count
    for veracity, date2count in list(veracity2date2count.items()):
        date_count = list(date2count.items())
        date_count.sort(key=lambda x: x[0])#datacount里面是啥data是啊金，根据时间排序
    ...
    for veracity, rid2count in list(veracity2rid2count.items()):
        rid_count = list(rid2count.items())
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        veracity = metadata['veracity']
        category = metadata['rumor_category']
        if category != 'Politics':#只有谣言的类型对了才会进行计数
            continue
    ...
    cat_count = list(category2count.items())#一共有多少种
    cat_count.sort(key=lambda x: x[1], reverse=True)#以数量排序
    y = [count for cat, count in cat_count]
    x = ['Politics', 'Urban Legends', 'Business', 'Terrorism & War', 'Science & Technology', 'Entertainment',
         'Natural Disasters']
    x.reverse()
    y.reverse()
    width = 0.015
    pos = [0.17, 0.19, 0.21, 0.23, 0.25, 0.27, 0.29]#这是已经设置好长度了
    ax = plt.subplot(111)
    ...
    '''
    print("Preparing figure 2A...")#全部都log化
    ax = plt.subplot(111)
    ax.set_xscale('log')
    ax.set_yscale('log')
    formatter = plt.FuncFormatter(log_10_product)
    ...
    veracity2clusters = {}
    for cascade, metadata in list(cascade_id2metadata.items()):
        veracity = metadata['veracity']#对于与是哪个种类的推特
        rid = metadata['rumor_id']
        ...
        clusters.append(rid)#不断添加rid，不同的种类吧
        veracity2clusters[veracity] = clusters
    print('##############################')
    print('Depth Stats. FALSE/TRUE/MIXED')
    print('Mean (log)', np.mean(np.log10(veracity2depths['FALSE'])), np.mean(np.log10(veracity2depths['TRUE'])),
    ...
    for veracity, depths in list(veracity2depths.items()):#下面的代码是经典的画ccdf的方法
        if veracity == 'MIXED':
            continue
    ...
    print('Mean (log)', np.mean(np.log10(veracity2sizes['FALSE'])), np.mean(np.log10(veracity2sizes['TRUE'])),
          np.mean(np.log10(veracity2sizes['MIXED'])))
    ...
    for veracity, sizes in list(veracity2sizes.items()):
        if veracity == 'MIXED':
            continue
    ...
    print('Mean (log)', np.mean(np.log10(veracity2breadths['FALSE'])), np.mean(np.log10(veracity2breadths['TRUE'])),
    ...
    for veracity, breadths in list(veracity2breadths.items()):
        if veracity == 'MIXED':
            continue
    ...
    print('Mean (log)', np.mean(np.log10([e for e in veracity2viralities['FALSE'] if e != None])),
          np.mean(np.log10([e for e in veracity2viralities['TRUE'] if e != None])),
          np.mean(np.log10([e for e in veracity2viralities['MIXED'] if e != None])))
    print('Robust SEM (log)', calc_robust_sem(np.log10([e for e in veracity2viralities['FALSE'] if e != None]),
                                              veracity2virality_clusters['FALSE']),
          calc_robust_sem(np.log10([e for e in veracity2viralities['TRUE'] if e != None]),
                          veracity2virality_clusters['TRUE']),
          calc_robust_sem(np.log10([e for e in veracity2viralities['MIXED'] if e != None]),
                          veracity2virality_clusters['MIXED']))
    print("KS test (false and true):", stats.ks_2samp([e for e in veracity2viralities['FALSE'] if e != None],
                                                      [e for e in veracity2viralities['TRUE'] if e != None]))
    for veracity, viralities in list(veracity2viralities.items()):
        if veracity == 'MIXED':
            continue
        xf = []
        pf = []
        viralities = [e if e != None else 0 for e in viralities]#把none给替换成0
        ...
        while c < max(viralities) + step:
            counts.append(c)#一个阶梯一个阶梯加
            c += step
        counts.sort()
        for d in counts:
            ind = bisect.bisect_left(viralities, d)#好奇怪的操作，插入一个这个，是为了平滑吗
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        veracity = metadata['veracity']
        rid = metadata['rumor_id']
        depth2time = metadata['depth2time']#一个字典是吗，深度与时间的对应关系
        depth2times = veracity2depth2times.get(veracity, {})#从这取dt
        depth2clusters = veracity2depth2clusters.get(veracity, {})
        for depth, time_ in list(depth2time.items()):
            times = depth2times.get(depth, [])#有这个深度对应的时间列表，就返回 从dt取dp对应的time
            times.append(time_)#在时间列表内添加上这个时间
            depth2times[depth] = times#继续对应
            clusters = depth2clusters.get(depth, [])#不断添加对应深度的内容，对应的集合
            clusters.append(rid)
            depth2clusters[depth] = clusters
        veracity2depth2times[veracity] = depth2times#这样就完成了深度与时间对应关系的收集
    ...
    for veracity in ['TRUE', 'FALSE']:
        ...
        y = []
        errors = []
        for d, times in list(depth2times.items()):
            if len(times) > thresh:  # thresh is set to 2 in settings.py. There needs to be at least two points for each depth
                ...
                y.append(np.mean(np.log10(times))) #取log之后再取一个平均
                ##Calculting robust SEM##
                clusters = veracity2depth2clusters[veracity][d]#对应深度的rid
                y_r = np.log10(times)
                robust_sem = calc_robust_sem(y_r, clusters)
                ####
                errors.append(robust_sem)#误差吗怎么计算的
        y = np.array(y)
        x = np.array(x)
        errors = np.array(errors)#转化为np数组
        plt.plot(x, y, color=veracity2color[veracity], linewidth=lw)#未了稳健性，很不错的思路与方法
        plt.fill_between(x, y - errors, y + errors, alpha=0.2, color=veracity2color[veracity])#这个就是画出阴影面积的方法
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        for uu, time_ in list(uu2time.items()):#唯一使用者
    ...
    for veracity in ['TRUE', 'FALSE']:
        ...
        for uu, times in list(uu2times.items()):
            if len(times) > thresh:
                if uu % 100 != 0 and uu != max_uu:  # we are binning every 100 , otherwise there would be too many
                    continue               #points in the plot and it would not load quickly.
                x.append(uu)#每一一百个添加一次
                times = np.array(times) / 60.0  # convert to minutes
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        for depth, uu in list(depth2uu.items()):
    ...
    for veracity in ['TRUE', 'FALSE']:
        ...
        y = []
        errors = []
        for d, uus in list(depth2uus.items()):
            if len(uus) > thresh:
                x.append(d)
                uus = np.array(uus)
                y.append(np.mean(np.log10(uus)))
                ##Robust SEM##
                clusters = veracity2depth2clusters[veracity][d]#这个应该是以前就计算过
                y_r = np.log10(uus)
                robust_sem = calc_robust_sem(y_r, clusters)
                ####
                errors.append(robust_sem)
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        for depth, breadth in list(depth2breadth.items()):
    ...
    for veracity in ['TRUE', 'FALSE']:
        ...
        y = []
        errors = []
        for d, breadths in list(depth2breadths.items()):
    ...
    '''
    print("Preparing figure 3A...")#谣言中有关政治的占的数量
    ax = plt.subplot(111)
    ax.set_xscale('log')
    ax.set_yscale('log')
    formatter = plt.FuncFormatter(log_10_product)
    ...
    print('Robust SEM (log)', calc_robust_sem(np.log10(category2depths['Politics']), category2clusters['Politics']),
          calc_robust_sem(np.log10(category2depths['Other']), category2clusters['Other']))#将中一种rid的聚合在一起
    print("KS test:", stats.ks_2samp(category2depths['Politics'], category2depths['Other']))
    for category, depths in list(category2depths.items()):
        total = float(len(depths))
        xf = []
        pf = []
        depths.sort()
    ...
    plt.xlabel("Cascade Depth", fontsize=fs)
    plt.ylabel("CCDF (%)", fontsize=fs)
    ax.tick_params(axis='both', which='major', labelsize=fs_2)#画折线图设置坐标轴
    plt.xticks(rotation='horizontal')
    ...
    for category, sizes in list(category2sizes.items()):
        total = float(len(sizes))
        xf = []
        pf = []
        sizes.sort()
    ...
    for category, breadths in list(category2breadths.items()):
        total = float(len(breadths))
        xf = []
        pf = []
        breadths.sort()
    ...
    print('Mean (log)', np.mean(np.log10([e for e in category2viralities['Politics'] if e != None])),
          np.mean(np.log10([e for e in category2viralities['Other'] if e != None])))
    print('Robust SEM (log)', calc_robust_sem(np.log10([e for e in category2viralities['Politics'] if e != None]),
                                              category2virality_clusters['Politics']),
          calc_robust_sem(np.log10([e for e in category2viralities['Other'] if e != None]),
                          category2virality_clusters['Other']))
    print("KS test:", stats.ks_2samp([e for e in category2viralities['Politics'] if e != None],
                                     [e for e in category2viralities['Other'] if e != None]))
    for category, viralities in list(category2viralities.items()):
        xf = []
        pf = []
    ...
    for category in ['Politics', 'Other']:
        ...
        y = np.array(y)
        x = np.array(x)
        errors = np.array(errors)#为什么有的加error有的不加
    ...
    '''
    print("Preparing figure 3F...")#y和x同时进行了变换
    ax = plt.subplot(111)
    formatter = plt.FuncFormatter(lognormal)
    ax.yaxis.set_major_formatter(formatter)
    ax.yaxis.set_major_locator(MaxNLocator(integer=True))
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        for uu, time_ in list(uu2time.items()):
            times = uu2times.get(uu, [])
            times.append(time_)
            uu2times[uu] = times
            clusters = uu2clusters.get(uu, [])
    ...
    for category in ['Politics', 'Other']:
        ...
        for uu, times in list(uu2times.items()):
            if len(times) > thresh:
                if uu % 100 != 0 and uu != max_uu:  # we are binning every 100 , otherwise there would be too
                    continue                        #many points in the plot and it would not load quickly.
                x.append(uu)
                times = np.array(times) / 60.0  # convert to minutes
                y.append(np.mean(np.log10(times)))
                ##Robust SEM##
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        followers = [e + epsilon for e in followers if e != None]  # smoothing#平滑要求，为什么要平滑
    ...
    D, p = stats.ks_2samp(veracity2followers['FALSE'], veracity2followers['TRUE'])#不需要按照顺序排列好，需要可以直接调用属性
    ...
    table = pd.DataFrame(table, ["followers", "followees", "verified", "engagement", "account age"],
                         ["median-false", "median-true", "mean-false", "mean-true", "mean(log)-false", "mean(log)-true",
                          "stdv(log)-false", "stdv(log)-true", "D", "p"])
    print(table)
    ...
    for tweet_id, rid_veracity_tweettopics_backgroundtopics in list(
            tid2rid_veracity_tweettopics_backgroundtopics.items()):
    ...
    for veracity in ['TRUE', 'FALSE']:
        ...
        means = np.array(
            [np.mean(ius), np.mean(kls_scaled), np.mean(bahs)])  # for visualization purposes we rescale the KL
        ##Robust SEM##
    ...
    for metric in ['IU', 'KL', 'BD']:
        D, p = stats.ks_2samp(veracity2novelty_metric2scores['FALSE'][metric],
                              veracity2novelty_metric2scores['TRUE'][metric])
        row = [np.mean(veracity2novelty_metric2scores['FALSE'][metric]),
               np.mean(veracity2novelty_metric2scores['TRUE'][metric]),
               np.var(veracity2novelty_metric2scores['FALSE'][metric]),
               np.var(veracity2novelty_metric2scores['TRUE'][metric]),
               D, p]
    ...
    for cascade, metadata in list(cascade_id2metadata.items()):
        ...
        if emotion2score != None:
            ...
            for emotion, score in list(emotion2score.items()):
    ...
    table = pd.DataFrame(table,
                         list[reversed(['trust', 'joy', 'anticipation', 'sadness', 'anger', 'fear', 'disgust', 'surprise'])],
                         ["mean-false", "mean-true", "var-false", "var-true", "D", "p"])
    print(table)
